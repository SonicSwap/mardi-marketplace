...
interface IHarmonyAddressRegistry {
    function mardi() external view returns (address);
...
interface IHarmonyBundleMarketplace {
...
interface IHarmonyNFTFactory {
    function exists(address) external view returns (bool);
}

interface IHarmonyTokenRegistry {
    function enabled(address) external view returns (bool);
}

interface IHarmonyPriceFeed {
    function wONE() external view returns (address);

    function getPrice(address) external view returns (int256, uint8);
}

contract HarmonyMarketplace is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    IHarmonyAddressRegistry public addressRegistry;
    ...
    function _buyItem(
        ...
        IHarmonyBundleMarketplace(addressRegistry.bundleMarketplace())
    ...
    function createOffer(
        ...
        IHarmonyAuction auction = IHarmonyAuction(addressRegistry.auction());
    ...
    function registerRoyalty(
        ...
        require(_isHarmonyNFT(_nftAddress), "invalid nft address");
    ...
    function _isHarmonyNFT(address _nftAddress) internal view returns (bool) {
        return
            addressRegistry.mardi() == _nftAddress ||
            IHarmonyNFTFactory(addressRegistry.factory()).exists(_nftAddress) ||
            IHarmonyNFTFactory(addressRegistry.privateFactory()).exists(
                _nftAddress
            ) ||
            IHarmonyNFTFactory(addressRegistry.artFactory()).exists(
                _nftAddress
            ) ||
            IHarmonyNFTFactory(addressRegistry.privateArtFactory()).exists(
                _nftAddress
            );
    }
    ...
    function getPrice(address _payToken) public view returns (int256) {
        int256 unitPrice;
        uint8 decimals;
        IHarmonyPriceFeed priceFeed = IHarmonyPriceFeed(
            addressRegistry.priceFeed()
        );

        if (_payToken == address(0)) {
            (unitPrice, decimals) = priceFeed.getPrice(priceFeed.wONE());
        } else {
    ...
    /**
     @notice Update HarmonyAddressRegistry contract
     @dev Only admin
     */
    function updateAddressRegistry(address _registry) external onlyOwner {
        addressRegistry = IHarmonyAddressRegistry(_registry);
    }
        ...
        require(
            _payToken == address(0) ||
                (addressRegistry.tokenRegistry() != address(0) &&
                    IHarmonyTokenRegistry(addressRegistry.tokenRegistry())
                        .enabled(_payToken)),
            "invalid pay token"
        );
    }
